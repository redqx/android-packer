...
DPT_ENCRYPT void mergeDexElement(JNIEnv* env,jclass __unused, jobject targetClassLoader,const char* pathChs) {
    ...
    DLOGD("mergeDexElement success. now the length is %d",env->GetArrayLength(newDexElements));
}
extern "C"
JNIEXPORT void JNICALL
Java_com_luoyesiqiu_shell_JniBridge_mergeDexElements(
//DPT_ENCRYPT void mergeDexElements(
JNIEnv* env,jclass klass, jobject targetClassLoader)
{
...
JNIEXPORT jstring JNICALL
        Java_com_luoyesiqiu_shell_JniBridge_readAppComponentFactory(
//        DPT_ENCRYPT jstring readAppComponentFactory(
JNIEnv *env, jclass __unused)
{
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_luoyesiqiu_shell_JniBridge_readApplicationName(
//DPT_ENCRYPT jstring readApplicationName(
JNIEnv *env, jclass __unused)
{
...
DPT_ENCRYPT jobject getApplicationInstance(JNIEnv *env, jstring applicationClassName)
{
    ...
    {
        const char *applicationClassNameChs = env->GetStringUTFChars(applicationClassName, nullptr);//原始的application名字
...
extern "C"
JNIEXPORT void JNICALL
Java_com_luoyesiqiu_shell_JniBridge_callRealApplicationOnCreate(
//DPT_ENCRYPT void callRealApplicationOnCreate(
JNIEnv *env, jclass, jstring realApplicationClassName)
{
...
extern "C"
JNIEXPORT void JNICALL
Java_com_luoyesiqiu_shell_JniBridge_callRealApplicationAttach(
//        DPT_ENCRYPT void callRealApplicationAttach(
JNIEnv *env, jclass, jobject context,jstring realApplicationClassName)
{
...
Java_com_luoyesiqiu_shell_JniBridge_replaceApplication(
//        DPT_ENCRYPT jobject replaceApplication(
JNIEnv *env, jclass klass, jstring realApplicationClassName)
{

    jobject appInstance = getApplicationInstance(env, realApplicationClassName);//通过findclass找到,然后调用它的构造函数并返回application
...
DPT_ENCRYPT void replaceApplicationOnLoadedApk(JNIEnv *env, jclass __unused,jobject realApplication) {
    ...
    //make it null
    loadedApk.setApplication(nullptr);//Y.mBoundApplication(AppBindData).info(LoadedApk).mApplication(Application)设置为null

    jobject mAllApplicationsObj = activityThread.getAllApplication();

    java_util_ArrayList arrayList(env,mAllApplicationsObj);

    jobject removed = (jobject)arrayList.remove(0);//从Y.mAllApplications(ArrayList<Application>)中移除Y.mInitialApplication(Application)
    if(removed != nullptr){
    ...
    //replace class name
    applicationInfo.setClassName(realApplicationNameGlobal);//把Y.mBoundApplication(AppBindData).info(LoadedApk).mApplicationInfo(ApplicationInfo).classname为源apk的ApplicationInfo的name
    appInfo.setClassName(realApplicationNameGlobal);//把Y.mBoundApplication(AppBindData).appInfo(ApplicationInfo).classname为源apk的ApplicationInfo的name
...
extern "C"
JNIEXPORT void JNICALL
Java_com_luoyesiqiu_shell_JniBridge_init_1app(
//        DPT_ENCRYPT void init_app(
JNIEnv *env, jclass __unused)
{
    DLOGD("init_app!");
    clock_t start = clock();
...
//    }
    ...
    extractDexesInNeeded(env,apk_addr,apk_size);//读取dex: i11111i111.zip,并写入到指定空间"/data/user/0/com.luoye.dpt/code_cache/i11111i111.zip"
